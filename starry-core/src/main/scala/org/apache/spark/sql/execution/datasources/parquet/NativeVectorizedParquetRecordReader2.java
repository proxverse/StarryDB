 ...
 */
public class NativeVectorizedParquetRecordReader2 extends SpecificParquetRecordReaderBase<Object> {
  ...
  public NativeVectorizedParquetRecordReader2(
      ZoneId convertTz,
  ...
  // For test only.
  public NativeVectorizedParquetRecordReader2(boolean useOffHeap, int capacity) {
    this(
        null,
        "CORRECTED",
        "UTC",
  ...
  public Object getCurrentValue() {
    if (returnColumnarBatch) {
      return VeloxColumnarBatch.createFromRowVector(((VeloxColumnarBatch) columnarBatch).rowVector(), batchSchema);
    }
  ...
  private void initBatch(
    ...
    ColumnVector[] vectors = initParquetVector(memMode);

    if (partitionColumns != null) {
      int partitionIdx = sparkSchema.fields().length;
      for (int i = 0; i < partitionColumns.fields().length; i++) {
        ColumnVectorUtils.populate((WritableColumnVector) vectors[i + partitionIdx], partitionValues, i);
        ((WritableColumnVector) vectors[i + partitionIdx]).setIsConstant();
      }
    }
  }

  private ColumnVector[] initParquetVector(MemoryMode memMode) {
    if (columnarBatch == null) {
      columnarBatch = ColumnBatchUtils.createWriterableColumnBatch(capacity, batchSchema);
    }
    ColumnVector[] vectors = ((VeloxColumnarBatch) columnarBatch).getColumns();

    ParquetColumnVector[] newColumnVectors = new ParquetColumnVector[sparkSchema.fields().length];
    for (int i = 0; i < newColumnVectors.length; i++) {
      newColumnVectors[i] = new ParquetColumnVector(parquetColumn.children().apply(i),
          (WritableColumnVector) vectors[i], capacity, memMode, missingColumns);
      if (columnVectors != null) {
        newColumnVectors[i].setColumnReader(columnVectors[i].getColumnReader());
      }
    }
