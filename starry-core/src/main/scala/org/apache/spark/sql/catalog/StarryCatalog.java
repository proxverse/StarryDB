 ...
 */
@Evolving
public class StarryCatalog implements CatalogExtension, FunctionCatalog {
  ...
  @Override
  public org.apache.spark.sql.connector.catalog.Table loadTable(Identifier ident, long timestamp) throws NoSuchTableException {
    return asTableCatalog().loadTable(ident, timestamp);
  }

  @Override
  public org.apache.spark.sql.connector.catalog.Table loadTable(Identifier ident, String version) throws NoSuchTableException {
  ...
  public UnboundFunction loadFunction(Identifier ident) throws NoSuchFunctionException {
    if (functionCache.containsKey(ident.name())) {
      return new StarryUnboundFunction(ident.name());
    }
  ...
  ConcurrentHashMap<String, Boolean> functionCache = new ConcurrentHashMap<>();

  @Override
  public boolean functionExists(Identifier ident) {
    if (!asFunctionCatalog().functionExists(ident)) {
      if (functionCache.containsKey(ident.name())) {
        return functionCache.get(ident.name());
      }
      boolean b = NativeExpressionConvert.nativeFunctionExists(ident.name());
      functionCache.put(ident.name(), b);
      return b;
    }
    return true;
  }
