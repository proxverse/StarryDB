...
class ArithmeticExpressionSuite extends SparkFunSuite with ColumnarExpressionEvalHelper {
...
//    testFunc(Decimal(_))
  ...
  ignore("Add: Overflow exception should contain SQL text context") {
    Seq(IntegerType, LongType).foreach { dt =>
      ...
      withOrigin(o) {
        ...
        checkExceptionInExpression[RuntimeException](
          expr,
          EmptyRow,
          s"${getMaxValue(dt)} + ${getMaxValue(dt)}")
      }
    }
  }

  override protected def checkExceptionInExpression[T <: Throwable: ClassTag](
    ...
    checkException(checkEvaluation(expression, inputRow), "")
  }
...
//    }
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "true") {
      checkExceptionInExpression[Throwable](
        UnaryMinus(Literal(Long.MinValue)),
        "Cannot negate minimum value")
      checkExceptionInExpression[Throwable](
        UnaryMinus(Literal(Int.MinValue)),
        "Cannot negate minimum value")
      checkExceptionInExpression[Throwable](
        UnaryMinus(Literal(Short.MinValue)),
        "Cannot negate minimum value")
      checkExceptionInExpression[Throwable](
        UnaryMinus(Literal(Byte.MinValue)),
        "Cannot negate minimum value")
  ...
  ignore("Minus: Overflow exception should contain SQL text context") {
    Seq(IntegerType, LongType).foreach { dt =>
      ...
      withOrigin(o) {
        ...
        checkExceptionInExpression[RuntimeException](
          expr,
          EmptyRow,
          s"${getMinValue(dt)} - ${getMaxValue(dt)}")
      }
    }
  }
  ...
  ignore("Multiply: Overflow exception should contain SQL text context") {
    Seq(IntegerType, LongType).foreach { dt =>
      val maxValue = getMaxValue(dt)
      val query = s"${maxValue.sql} * ${maxValue.sql}"
      val o = Origin(
      ...
      withOrigin(o) {
        ...
        checkExceptionInExpression[RuntimeException](expr, EmptyRow, s"${maxValue} * ${maxValue}")
      }
    }
  }
...
//    }
  ...
  //       ANSI_ENABLED = true
  ignore("Divide: divide by 0 exception should contain SQL text context") {
    val query = "1234.5D / 0"
    val o = Origin(
      line = Some(1),
      startPosition = Some(7),
    ...
    withOrigin(o) {
      ...
      checkExceptionInExpression[RuntimeException](expr, EmptyRow, query)
    }
  }
...
//    testFunc(Decimal(_))
  ...
  ignore("/ (Divide) for Long and Decimal type") {
...
//      }
  ...
  ignore("IntegralDivide: throw exception on overflow under ANSI mode") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "true") {
      checkExceptionInExpression[RuntimeException](
  ...
  ignore("IntegralDivide: exception should contain SQL text context") {
    Seq(-1L, 0L).foreach { right =>
      ...
      withOrigin(o) {
        val expr =
          IntegralDivide(
            Literal(Long.MinValue, LongType),
            Literal(right, LongType),
            failOnError = true)
        checkExceptionInExpression[RuntimeException](expr, EmptyRow, query)
      }
    }
...
//      }
  ...
  // ansi
  ignore("Remainder/Pmod: exception should contain SQL text context") {
    Seq(("%", Remainder), ("pmod", Pmod)).foreach {
      case (symbol, exprBuilder) =>
        ...
        withOrigin(o) {
          ...
          checkExceptionInExpression[RuntimeException](expression, EmptyRow, query)
        }
    }
  }
...
//    withSQLConf(SQLConf.ANSI_ENABLED.key -> "true") {
//      minValues.foreach { v =>
//        checkExceptionInExpression[RuntimeException](Abs(v), "overflow")
//      }
//    }
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "false") {
      minValues.foreach { v =>
...
//    checkEvaluation(Pmod(Literal(Decimal(0.7)), Literal(Decimal(0.2))), Decimal(0.1))
  ...
  test("function least") {
    ...
    val c4 = 'd.string
    val c5 = 'e.string
    val schema = StructType.fromAttributes(Seq(c1, c2, c3, c4, c5))
    checkEvaluation(Least(Seq(c4, c3, c5)), "a", row, schema)
    checkEvaluation(Least(Seq(c1, c2)), 1, row, schema)
    checkEvaluation(Least(Seq(c1, c2, Literal(-1))), -1, row, schema)
    checkEvaluation(Least(Seq(c4, c5, c3, c3, Literal("a"))), "a", row, schema)
...
//    checkEvaluation(least, Seq(1, 2))
  ...
  test("function greatest") {
    ...
    checkEvaluation(Greatest(Seq(c4, c5, c3)), "c", row, schema)
    checkEvaluation(Greatest(Seq(c2, c1)), 2, row, schema)
    checkEvaluation(Greatest(Seq(c1, c2, Literal(2))), 2, row, schema)
    checkEvaluation(Greatest(Seq(c4, c5, c3, Literal("ccc"))), "ccc", row, schema)
...
//      InternalRow.empty)
    checkEvaluation(
      Greatest(Seq(Literal(Date.valueOf("2015-01-01")), Literal(Date.valueOf("2015-07-01")))),
...
//        checkExceptionInExpression[RuntimeException](e, "overflow")
//      }
//    }
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "false") {
      val e1 = Add(maxLongLiteral, Literal(1L))
...
//    }
  ...
  ignore("SPARK-33008: division by zero on divide-like operations returns incorrect result") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "true") {
      val operators
        : Seq[((Expression, Expression) => Expression, ((Int => Any) => Unit) => Unit)] =
        Seq(
          (Divide(_, _), testDecimalAndDoubleType),
          (IntegralDivide(_, _), testDecimalAndLongType),
          (Remainder(_, _), testNumericDataTypes),
          (Pmod(_, _), testNumericDataTypes))
      operators.foreach {
        case (operator, testTypesFn) =>
          testTypesFn { convert =>
            ...
            checkExceptionInExpression[RuntimeException](operator(one, zero), "Division by zero")
          }
      }
    }
  }

  ignore("SPARK-34677: exact add and subtract of day-time and year-month intervals") {
    Seq(true, false).foreach { failOnError =>
      checkExceptionInExpression[RuntimeException](
        UnaryMinus(
  ...
  ignore("SPARK-34920: error class") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "true") {
      val operators
        : Seq[((Expression, Expression) => Expression, ((Int => Any) => Unit) => Unit)] =
        Seq(
  ...
  ignore("SPARK-36920: Support year-month intervals by ABS") {
  ...
  ignore("SPARK-36920: Support day-time intervals by ABS") {
  ...
  ignore("SPARK-36921: Support YearMonthIntervalType by div") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "false") {
      checkEvaluation(IntegralDivide(Literal(Period.ZERO), Literal(Period.ZERO)), null)
      checkEvaluation(IntegralDivide(Literal(Period.ofYears(1)), Literal(Period.ZERO)), null)
      checkEvaluation(IntegralDivide(Period.ofMonths(Int.MinValue), Literal(Period.ZERO)), null)
      checkEvaluation(IntegralDivide(Period.ofMonths(Int.MaxValue), Literal(Period.ZERO)), null)
    }
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "true") {
      checkExceptionInExpression[RuntimeException](
        IntegralDivide(Literal(Period.ZERO), Literal(Period.ZERO)),
        "Division by zero")
      checkExceptionInExpression[RuntimeException](
    ...
    checkEvaluation(
      ...
      null)
    checkEvaluation(
      IntegralDivide(Literal.create(null, YearMonthIntervalType()), Literal(Period.ofYears(1))),
      null)
    checkEvaluation(
      IntegralDivide(Literal(Period.ofYears(1)), Literal.create(null, YearMonthIntervalType())),
      null)

    checkEvaluation(
      IntegralDivide(Period.ofMonths(Int.MaxValue), Period.ofMonths(Int.MaxValue)),
      1L)
    checkEvaluation(
      IntegralDivide(Period.ofMonths(Int.MaxValue), Period.ofMonths(Int.MinValue)),
      0L)
    checkEvaluation(
      IntegralDivide(Period.ofMonths(Int.MinValue), Period.ofMonths(Int.MinValue)),
      1L)
    checkEvaluation(
      IntegralDivide(Period.ofMonths(Int.MinValue), Period.ofMonths(Int.MaxValue)),
      -1L)

    checkEvaluation(IntegralDivide(Literal(Period.ZERO), Literal(Period.ofYears(-1))), 0L)
    checkEvaluation(IntegralDivide(Literal(Period.ofYears(2)), Literal(Period.ofYears(1))), 2L)
    checkEvaluation(IntegralDivide(Literal(Period.ofYears(2)), Literal(Period.ofYears(-1))), -2L)
    checkEvaluation(IntegralDivide(Literal(Period.ofYears(1)), Literal(Period.ofMonths(3))), 4L)
    checkEvaluation(IntegralDivide(Literal(Period.ofYears(1)), Literal(Period.ofMonths(-3))), -4L)
    checkEvaluation(IntegralDivide(Literal(Period.ofYears(1)), Literal(Period.ofMonths(5))), 2L)
    checkEvaluation(IntegralDivide(Literal(Period.ofYears(1)), Literal(Period.ofMonths(-5))), -2L)
  }
  ignore("SPARK-36921: Support DayTimeIntervalType by div") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "false") {
      checkEvaluation(IntegralDivide(Literal(Duration.ZERO), Literal(Duration.ZERO)), null)
      checkEvaluation(IntegralDivide(Literal(Duration.ofDays(1)), Literal(Duration.ZERO)), null)
      checkEvaluation(
        IntegralDivide(
          Literal(Duration.of(Long.MaxValue, ChronoUnit.MICROS)),
          Literal(Duration.ZERO)),
        null)
      checkEvaluation(
        IntegralDivide(
          Literal(Duration.of(Long.MinValue, ChronoUnit.MICROS)),
          Literal(Duration.ZERO)),
        null)
    }
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "true") {
      checkExceptionInExpression[RuntimeException](
        ...
        "Division by zero")
      checkExceptionInExpression[RuntimeException](
        IntegralDivide(Literal(Duration.ofDays(1)), Literal(Duration.ZERO)),
        "Division by zero")
      checkExceptionInExpression[RuntimeException](
        IntegralDivide(
          Literal(Duration.of(Long.MaxValue, ChronoUnit.MICROS)),
    ...
    checkEvaluation(
      ...
      null)
    checkEvaluation(
      IntegralDivide(Literal.create(null, DayTimeIntervalType()), Literal(Duration.ofDays(1))),
      null)
    checkEvaluation(
      IntegralDivide(Literal(Duration.ofDays(1)), Literal.create(null, DayTimeIntervalType())),
      null)
    ...
    checkEvaluation(IntegralDivide(Literal(Duration.ZERO), Literal(Duration.ofDays(-1))), 0L)
    checkEvaluation(IntegralDivide(Literal(Duration.ofDays(2)), Literal(Duration.ofDays(1))), 2L)
    checkEvaluation(
      IntegralDivide(Literal(Duration.ofDays(2)), Literal(Duration.ofDays(-1))),
      -2L)
    checkEvaluation(IntegralDivide(Literal(Duration.ofDays(1)), Literal(Duration.ofHours(4))), 6L)
    checkEvaluation(
      IntegralDivide(Literal(Duration.ofDays(1)), Literal(Duration.ofHours(-4))),
      -6L)
    checkEvaluation(IntegralDivide(Literal(Duration.ofDays(1)), Literal(Duration.ofHours(5))), 4L)
    checkEvaluation(
      IntegralDivide(Literal(Duration.ofDays(1)), Literal(Duration.ofHours(-5))),
      -4L)
  }
}
