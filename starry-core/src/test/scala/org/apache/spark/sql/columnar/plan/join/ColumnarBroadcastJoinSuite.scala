  ...
  test("broadcast join where streamed side's output partitioning is HashPartitioning") {
    withTable("t1", "t3") {
      withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "500") {
        ...
        assert(collect(plan2) { case s: ShuffledHashJoinExec => s }.size == 1)
...
class BroadcastJoinSuite extends BroadcastJoinSuiteBase   {
  override protected def test(testName: String, testTags: Tag*)(testFun: => Any)
      (implicit pos: Position): Unit = {
    if (testName.contains("broadcast join where streamed side's output partitioning is HashPartitioning")) {
