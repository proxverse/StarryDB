 ...
 */
abstract class CastSuiteBase extends SparkFunSuite with ColumnarExpressionEvalHelper {
  ...
  protected def verifyCastFailure(
      c: CastBase,
      optionalExpectedMsg: Option[String] = None): Unit = {
  ...
  test("cast string to timestamp") {
    Seq(getZoneId("Asia/Shanghai")).foreach { zid =>
...
//    }
  ...
  ignore("cast struct with a timestamp field") {
  ...
  ignore("cast between string and interval") {
  ...
  ignore("SPARK-16729 type checking for casting to date type") {
  ...
  ignore("SPARK-20302 cast with same structure") {
  ...
  test("up-cast") {
    ...
    numericTypes.foreach { from =>
      ...
      safeTargetTypes.foreach { to =>
        ...
        Seq(true -> true, false -> false, false -> true).foreach {
          case (fn, tn) =>
  ...
  ignore("SPARK-27671: cast from nested null type in struct") {
  ...
  ignore("Process Infinity, -Infinity, NaN in case insensitive manner") {
  ...
  // nulltype
  ignore("SPARK-33291: Cast array with null elements to string") {
    Seq(false, true).foreach { omitNull =>
      withSQLConf(SQLConf.LEGACY_COMPLEX_TYPES_TO_STRING.key -> omitNull.toString) {
  ...
  ignore("SPARK-22973 Cast map to string") {
    Seq(false -> ("{", "}"), true -> ("[", "]")).foreach {
      case (legacyCast, (lb, rb)) =>
        withSQLConf(SQLConf.LEGACY_COMPLEX_TYPES_TO_STRING.key -> legacyCast.toString) {
          ...
          val ret5 =
            cast(Literal.create(Map(1 -> Array(1, 2, 3), 2 -> Array(4, 5, 6))), StringType)
          checkEvaluation(ret5, s"${lb}1 -> [1, 2, 3], 2 -> [4, 5, 6]$rb")
        }
    }
  }

  ignore("SPARK-22981 Cast struct to string") {
    Seq(false -> ("{", "}"), true -> ("[", "]")).foreach {
      case (legacyCast, (lb, rb)) =>
        withSQLConf(SQLConf.LEGACY_COMPLEX_TYPES_TO_STRING.key -> legacyCast.toString) {
          ...
          val ret3 = cast(
            Literal.create(
              (Date.valueOf("2014-12-03"), Timestamp.valueOf("2014-12-03 15:05:00"))),
            StringType)
  ...
  ignore("SPARK-33291: Cast struct with null elements to string") {
    Seq(false -> ("{", "}"), true -> ("[", "]")).foreach {
      case (legacyCast, (lb, rb)) =>
        withSQLConf(SQLConf.LEGACY_COMPLEX_TYPES_TO_STRING.key -> legacyCast.toString) {
  ...
  ignore("SPARK-34667: cast year-month interval to string") {
    Seq(
      ...
      Period.ofMonths(Int.MinValue) -> "-178956970-8").foreach {
  ...
  ignore("SPARK-34668: cast day-time interval to string") {
    Seq(
  ...
  ignore("SPARK-35698: cast timestamp without time zone to string") {
    specialTs.foreach { s =>
      checkEvaluation(cast(LocalDateTime.parse(s), StringType), s.replace("T", " "))
    }
  }

  ignore("SPARK-35711: cast timestamp without time zone to timestamp with local time zone") {
  ...
  ignore("SPARK-35716: cast timestamp without time zone to date type") {
    specialTs.foreach { s =>
  ...
  ignore("SPARK-35718: cast date type to timestamp without timezone") {
    specialTs.foreach { s =>
  ...
  ignore("SPARK-35719: cast timestamp with local time zone to timestamp without timezone") {
    outstandingZoneIds.foreach { zoneId =>
      withDefaultTimeZone(zoneId) {
        specialTs.foreach { s =>
  ...
  ignore("disallow type conversions between Numeric types and Timestamp without time zone type") {
  ...
  ignore("SPARK-35720: cast string to timestamp without timezone") {
    specialTs.foreach { s =>
      val expectedTs = LocalDateTime.parse(s)
  ...
  ignore("SPARK-35112: Cast string to day-time interval") {
  ...
  ignore("SPARK-35111: Cast string to year-month interval") {
    checkEvaluation(
  ...
  ignore("SPARK-35820: Support cast DayTimeIntervalType in different fields") {
  ...
  ignore("SPARK-35819: Support cast YearMonthIntervalType in different fields") {
  ...
  ignore("SPARK-35768: Take into account year-month interval fields in cast") {
    Seq(
      ("1-1", YearMonthIntervalType(YEAR, MONTH), 13),
      ("-1-1", YearMonthIntervalType(YEAR, MONTH), -13))
      .foreach {
        case (str, dataType, ym) =>
        ...
        assert(
          e.contains(
            s"Interval string does not match year-month format of " +
              s"${IntervalUtils
                .supportedFormat((dataType.startField, dataType.endField))
                .map(format => s"`$format`")
                .mkString(", ")} " +
  ...
  ignore("SPARK-35735: Take into account day-time interval fields in cast") {
