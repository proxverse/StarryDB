...
class MathExpressionsSuite extends SparkFunSuite with ColumnarExpressionEvalHelper {
  ...
  def toRound(value: String, scale: Int = 10): Double = {
    BigDecimal.apply(value).setScale(scale, RoundingMode.HALF_UP).toDouble
  }
  ...
  private def testUnary[T, U](
      ...
      f: T => U,
      domain: Iterable[T] = (-20 to 20).map(_ * 0.1).map(_.asInstanceOf[T]),
      expectNull: Boolean = false,
      expectNaN: Boolean = false,
      evalType: DataType = DoubleType): Unit = {
    if (expectNull) {
      domain.foreach { value =>
        checkEvaluation(Round(c(Literal(value)), 10), null, EmptyRow)
      }
    } else if (expectNaN) {
      domain.foreach { value =>
        checkNaN(Round(c(Literal(value)), 10), EmptyRow)
      }
    } else {
      domain.foreach { value =>
        try {
          checkEvaluation(Round(c(Literal(value)), 10), toRound(f(value).toString), EmptyRow)
        } catch {
    ...
    checkEvaluation(Round(c(Literal.create(null, evalType)), 13), null, create_row(null))
  }

  private def testUnaryString[T, U](
      c: Expression => Expression,
      f: T => U,
      domain: Iterable[T] = (-20 to 20).map(_ * 0.1).map(_.asInstanceOf[T]),
      expectNull: Boolean = false,
  ...
  private def testBinary(
    ...
    if (expectNull) {
      domain.foreach {
        case (v1, v2) =>
          checkEvaluation(Round(c(Literal(v1), Literal(v2)), 1), null, create_row(null))
      }
    } else if (expectNaN) {
      domain.foreach {
        case (v1, v2) =>
      ...
      domain.foreach {
        case (v1, v2) =>
          checkEvaluation(
            Round(c(Literal(v1), Literal(v2)), 1),
            toRound(f(v1 + 0.0, v2 + 0.0).toString, 1),
            EmptyRow)
          checkEvaluation(
            Round(c(Literal(v2), Literal(v1)), 1),
            toRound(f(v2 + 0.0, v1 + 0.0).toString, 1),
            EmptyRow)
      }
    }
  ...
  private def checkNaNWithoutCodegen(
    ...
    val actual = try evaluateWithoutCodegen(expression, inputRow)
    catch {
  ...
  test("cot") {
    ...
    val intNullLit = Literal.create(null, IntegerType)
    val intLit = Literal.create(1, IntegerType)
    checkEvaluation(Round(checkDataTypeAndCast(Cot(nullLit)), 10), null, EmptyRow)
    checkEvaluation(Round(checkDataTypeAndCast(Cot(intNullLit)), 10), null, EmptyRow)
    checkEvaluation(
      Round(checkDataTypeAndCast(Cot(intLit)), 10),
      toRound((1 / math.tan(1)).toString),
      EmptyRow)
    checkEvaluation(
      Round(checkDataTypeAndCast(Cot(-intLit)), 10),
      toRound((1 / math.tan(-1)).toString),
      EmptyRow)
    checkEvaluation(Round(checkDataTypeAndCast(Cot(0)), 10), (1 / math.tan(0)), EmptyRow)
  }
...
//    checkConsistencyBetweenInterpretedAndCodegen(Floor, DecimalType(5, 0))
  ...
  test("bin") {
    ...
    val schema = StructType.fromAttributes(Seq(l1, l2, l3, l4, l5))
    checkEvaluation(Bin(l1), null, row, schema)
    checkEvaluation(Bin(l2), java.lang.Long.toBinaryString(12), row, schema)
    checkEvaluation(Bin(l3), java.lang.Long.toBinaryString(123), row, schema)
    checkEvaluation(Bin(l4), java.lang.Long.toBinaryString(1234), row, schema)
    checkEvaluation(Bin(l5), java.lang.Long.toBinaryString(-123), row, schema)
  ...
  test("hex") {
    ...
    checkEvaluation(Hex(Literal(28L)), "1C".toLowerCase())
    checkEvaluation(Hex(Literal(-28L)), "FFFFFFFFFFFFFFE4".toLowerCase())
    checkEvaluation(Hex(Literal(100800200404L)), "177828FED4".toLowerCase())
    checkEvaluation(Hex(Literal(-100800200404L)), "FFFFFFE887D7012C".toLowerCase())
...
//    checkEvaluation(Unhex(Literal("F")), Array[Byte](15))
    checkEvaluation(Unhex(Literal("F")), null)
  ...
  test("binary log") {
    ...
    domain.foreach {
      case (v1, v2) =>
        checkEvaluation(
          Round(Logarithm(Literal(v1), Literal(v2)), 6),
          if (f(v1 + 0.0, v2 + 0.0) == Double.NegativeInfinity || f(v1 + 0.0, v2 + 0.0) == Double.PositiveInfinity) {
            f(v1 + 0.0, v2 + 0.0)
          } else {
            BigDecimal(f(v1 + 0.0, v2 + 0.0)).setScale(6, RoundingMode.HALF_UP).toDouble
          },
          EmptyRow)
        checkEvaluation(
          Round(Logarithm(Literal(v2), Literal(v1)), 6),
          if (f(v2 + 0.0, v1 + 0.0) == Double.NegativeInfinity || f(v2 + 0.0, v1 + 0.0) == Double.PositiveInfinity) {
            f(v2 + 0.0, v1 + 0.0)
          } else {
            BigDecimal(f(v2 + 0.0, v1 + 0.0)).setScale(6, RoundingMode.HALF_UP).toDouble
          },
          EmptyRow)
        checkEvaluation(
          Round(new Logarithm(Literal(v1)), 6),
          if (f(math.E, v1 + 0.0) == Double.NegativeInfinity || f(math.E, v1 + 0.0) == Double.PositiveInfinity) {
            f(math.E, v1 + 0.0)
          } else {
            BigDecimal(f(math.E, v1 + 0.0)).setScale(6, RoundingMode.HALF_UP).toDouble
          },
          EmptyRow)
    }
  ...
  ignore("round/bround/floor/ceil") {
    val scales = -6 to 6
...
//        checkEvaluation(Round(floatPi, scale), floatResults(i), EmptyRow)
  ...
  ignore("SPARK-36922: Support ANSI intervals for SIGN/SIGNUM") {
  ...
  ignore("SPARK-35926: Support YearMonthIntervalType in width-bucket function") {
    Seq(
      ...
      .foreach {
        case ((v, s, e, n), expected) =>
          checkEvaluation(WidthBucket(Literal(v), Literal(s), Literal(e), Literal(n)), expected)
      }
  }

  ignore("SPARK-35925: Support DayTimeIntervalType in width-bucket function") {
    Seq(
      ...
      (
        ...
        10L) -> 6L,
      (Duration.ofDays(0), Duration.of(Long.MinValue, ChronoUnit.MICROS), Duration.ofDays(0), 10L) -> 11L,
      (
        Duration.of(Long.MinValue, ChronoUnit.MICROS),
        Duration.of(Long.MinValue, ChronoUnit.MICROS),
        Duration.ofDays(0),
