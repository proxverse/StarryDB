...
class ConditionalExpressionSuite extends SparkFunSuite with ColumnarExpressionEvalHelper {
...
//    testIf(_.toLong, TimestampType)  timezone
//
  ...
  test("case when") {
    ...
    checkEvaluation(CaseWhen(Seq((c1, c4)), c6), "c", row, schema)
    checkEvaluation(CaseWhen(Seq((c2, c4)), c6), "c", row, schema)
    checkEvaluation(CaseWhen(Seq((c3, c4)), c6), "a", row, schema)
    checkEvaluation(CaseWhen(Seq((Literal.create(null, BooleanType), c4)), c6), "c", row, schema)
    checkEvaluation(CaseWhen(Seq((Literal.create(false, BooleanType), c4)), c6), "c", row, schema)
    checkEvaluation(CaseWhen(Seq((Literal.create(true, BooleanType), c4)), c6), "a", row, schema)

    checkEvaluation(CaseWhen(Seq((c3, c4), (c2, c5)), c6), "a", row, schema)
    checkEvaluation(CaseWhen(Seq((c2, c4), (c3, c5)), c6), "b", row, schema)
    checkEvaluation(CaseWhen(Seq((c1, c4), (c2, c5)), c6), "c", row, schema)
    checkEvaluation(CaseWhen(Seq((c1, c4), (c2, c5))), null, row, schema)
...
//    checkResult(mapWithNulls.dataType, mapWithoutNulls.value, mapCaseWhen4)
  ...
  test("case key when") {
    ...
    checkEvaluation(CaseKeyWhen(c1, Seq(c2, c4, c5)), "b", row, schema)
    checkEvaluation(CaseKeyWhen(c1, Seq(c2, c4, literalNull, c5, c6)), "c", row, schema)
    checkEvaluation(CaseKeyWhen(c2, Seq(literalInt, c4, c5)), "a", row, schema)
    checkEvaluation(CaseKeyWhen(c2, Seq(c1, c4, c5)), "b", row, schema)
    checkEvaluation(CaseKeyWhen(c4, Seq(literalString, c2, c3)), 1, row, schema)
    checkEvaluation(CaseKeyWhen(c4, Seq(c6, c3, c5, c2, Literal(3))), 3, row, schema)

    checkEvaluation(CaseKeyWhen(literalInt, Seq(c2, c4, c5)), "a", row, schema)
    checkEvaluation(CaseKeyWhen(literalString, Seq(c5, c2, c4, c3)), 2, row, schema)
    checkEvaluation(CaseKeyWhen(c6, Seq(c5, c2, c4, c3)), null, row, schema)
    checkEvaluation(CaseKeyWhen(literalNull, Seq(c2, c5, c1, c6)), null, row, schema)
  }
  ...
  test("SPARK-27551: informative error message of mismatched types for case when") {
    ...
    assert(
      checkResult1
        .asInstanceOf[TypeCheckResult.TypeCheckFailure]
        .message
