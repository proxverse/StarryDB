...
/**
 *
|           | tinyint | smallint | integer | bigint | boolean | real | double | varchar | timestamp | timestamp with time zone | date | decimal |
|-----------|---------|----------|---------|--------|---------|------|--------|---------|-----------|-------------------------|------|---------|
...
class CastSuite
    ...
    with ColumnarExpressionEvalHelper {
  ...
  override protected def checkNullCast(from: DataType, to: DataType): Unit = {
    if (atomicTypes.contains(from) && atomicTypes.contains(to)) {
  ...
  test("cast string to int #1") {
    checkEvaluation(Cast(Literal("1234"), IntegerType), 1234)
    checkEvaluation(Cast(Literal("2015"), IntegerType), 2015)
    checkEvaluation(Cast(Literal("2015.03.18"), IntegerType), null)
  }

  test("cast string to long #1") {
    checkEvaluation(Cast(Literal("1234"), LongType), 1234)
    checkEvaluation(Cast(Literal("2015"), LongType), 2015)
    checkEvaluation(Cast(Literal("2015.03.18"), LongType), null)
  }
  ...
  ignore("casting to fixed-precision decimals") {
  ...
  ignore("SPARK-28470: Cast should honor nullOnOverflow property") {
  ...
  ignore("Cast should output null for invalid strings when ANSI is not enabled.") {
    withSQLConf(SQLConf.ANSI_ENABLED.key -> "false") {
  ...
  ignore("cast a timestamp before the epoch 1970-01-01 00:00:00Z") {
  ...
  ignore("SPARK-32828: cast from a derived user-defined type to a base type") {
    val v = Literal.create(Row(1), new ExampleSubTypeUDT())
    checkEvaluation(cast(v, new ExampleBaseTypeUDT), Row(1))
  }

  ignore("Fast fail for cast string type to decimal type") {
    checkEvaluation(
  ...
  ignore("SPARK-35720: cast invalid string input to timestamp without time zone") {
  ...
  ignore("SPARK-36924: Cast DayTimeIntervalType to IntegralType") {
  ...
  ignore("SPARK-36924: Cast IntegralType to DayTimeIntervalType") {
    Seq(
      ...
      (
        ...
        Short.MinValue * MICROS_PER_SECOND)).foreach {
    ...
    Seq(
      ...
      (Long.MinValue, DayTimeIntervalType(SECOND))).foreach {
      case (v, toType) =>
        checkExceptionInExpression[ArithmeticException](
          cast(v, toType),
          castOverflowErrMsg(toType))
  ...
  ignore("SPARK-36924: Cast YearMonthIntervalType to IntegralType") {
    Seq(
  ...
  ignore("SPARK-36924: Cast IntegralType to YearMonthIntervalType") {
    Seq(
      (0, 0, 0, ByteType),
      (0, 0, 0, ShortType),
      (0, 0, 0, IntegerType),
    ...
    Seq(
      ...
      (Long.MinValue, YearMonthIntervalType(MONTH))).foreach {
      case (v, toType) =>
        checkExceptionInExpression[ArithmeticException](
          cast(v, toType),
          castOverflowErrMsg(toType))
