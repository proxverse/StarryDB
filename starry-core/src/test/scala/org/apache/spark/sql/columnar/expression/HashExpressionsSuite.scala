...
import org.apache.spark.util.Utils.isMac
...
class HashExpressionsSuite extends SparkFunSuite with ColumnarExpressionEvalHelper {
...
//      expected = -1142817416)
  ...
  ignore("SPARK-22284: Compute hash for nested structs") {
    val M = 80
    val N = 10
    val L = M * N
    val O = 50
  ...
  ignore("SPARK-30633: xxHash with different type seeds") {
  ...
  ignore("SPARK-35113: HashExpression support DayTimeIntervalType/YearMonthIntervalType") {
  ...
  ignore("SPARK-35207: Compute hash consistent between -0.0 and 0.0") {
...
//      checkEvaluation(HiveHash(Seq(exprs1)), HiveHash(Seq(exprs2)).eval())
  ...
  private def testHash(inputSchema: StructType): Unit = {
    ...
    ignore(s"murmur3/xxHash64/hive hash: ${inputSchema.simpleString}") {
      for (_ <- 1 to 10) {
...
//        checkEvaluation(XxHash64(literals, seed), XxHash64(literals, seed).eval())
    ...
    ignore(s"SPARK-30633: xxHash64 with long seed: ${inputSchema.simpleString}") {
      for (_ <- 1 to 10) {
        val input = toRow(inputGenerator.apply().asInstanceOf[Row]).asInstanceOf[UnsafeRow]
        val literals = input.toSeq(inputSchema).zip(inputSchema.map(_.dataType)).map {
          case (value, dt) => Literal.create(value, dt)
