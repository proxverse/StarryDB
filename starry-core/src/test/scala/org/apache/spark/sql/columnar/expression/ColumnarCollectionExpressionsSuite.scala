...
import org.apache.spark.sql.catalyst.util.IntervalUtils.{fromDayTimeString, fromYearMonthString, negateExact, stringToInterval}
...
class ColumnarCollectionExpressionsSuite  extends ColumnarExpressionEvalHelper {
  self: Suite =>
...
//    checkEvaluation(ArraysOverlap(b0, arrayWithBinaryNull), null)
  ...
  ignore("SPARK-37544: Time zone should not affect date sequence with month interval") {
  ...
  ignore("SPARK-35088: Accept ANSI intervals by the Sequence expression") {
    checkEvaluation(new Sequence(
      Literal(Timestamp.valueOf("2018-01-01 00:00:00")),
      Literal(Timestamp.valueOf("2018-01-02 00:00:00")),
  ...
  ignore("SPARK-36090: Support TimestampNTZType in expression Sequence") {
    checkEvaluation(new Sequence(
  ...
  ignore("Sequence with default step") {
    // +/- 1 for integral type
...
//    assert(ArrayExcept(a24, a22).dataType.asInstanceOf[ArrayType].containsNull)
  ...
  ignore("SPARK-31980: Start and end equal in month range") {
    checkEvaluation(new Sequence(
      Literal(Date.valueOf("2018-01-01")),
      Literal(Date.valueOf("2018-01-01")),
      Literal(stringToInterval("interval 1 day"))),
  ...
  ignore("SPARK-36639: Start and end equal in month range with a negative step") {
    checkEvaluation(new Sequence(
      Literal(Date.valueOf("2018-01-01")),
      Literal(Date.valueOf("2018-01-01")),
      Literal(stringToInterval("interval -1 day"))),
  ...
  ignore("SPARK-33460: element_at NoSuchElementException") {
    Seq(true, false).foreach { ansiEnabled =>
      withSQLConf(SQLConf.ANSI_ENABLED.key -> ansiEnabled.toString) {
  ...
  ignore("SPARK-36702: ArrayUnion should handle duplicated Double.NaN and Float.Nan") {
  ...
  ignore("SPARK-36753: ArrayExcept should handle duplicated Double.NaN and Float.Nan") {
    checkEvaluation(ArrayExcept(
  ...
  ignore("SPARK-36754: ArrayIntersect should handle duplicated Double.NaN and Float.Nan") {
    checkEvaluation(ArrayIntersect(
  ...
  ignore("SPARK-36741: ArrayDistinct should handle duplicated Double.NaN and Float.Nan") {
  ...
  ignore("SPARK-36755: ArraysOverlap hould handle duplicated Double.NaN and Float.Nan") {
    checkEvaluation(ArraysOverlap(
  ...
  ignore("SPARK-36740: ArrayMin/ArrayMax/SortArray should handle NaN greater then non-NaN value") {
    // ArrayMin
  ...
  ignore("SPARK-39184: Avoid ArrayIndexOutOfBoundsException when crossing DST boundary") {
    DateTimeTestUtils.withDefaultTimeZone(LA) {
      checkEvaluation(new Sequence(
