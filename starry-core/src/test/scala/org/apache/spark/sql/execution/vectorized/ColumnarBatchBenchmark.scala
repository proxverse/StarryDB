...
object ColumnarBatchBenchmark extends BenchmarkBase {
  ...
  def intAccess(iters: Long): Unit = {
    ...
    benchmark.addCase("Column(Native)")(columnNative)
  ...
  def stringAccess(iters: Long): Unit = {
    ...
    def column(memoryMode: MemoryMode) = { i: Int =>
      ...
      for (n <- 0L until iters) {
        ...
        i = 0
        while (i < count) {
          sum += column.getUTF8String(i).getBytes().size
          i += 1
        }
        column.reset()
      }
    ...
    benchmark.addCase("Native")(column(null))
    benchmark.run
  }
  ...
  def arrayAccess(iters: Int): Unit = {
    ...
    def readArrays(onHeap: Int): Unit = {
      val vector = onHeap match {
        case 0 =>
          onHeapVector
        case 1 =>
          offHeapVector
    ...
    def readArrayElements(onHeap: Int): Unit = {
      val vector = onHeap match {
        case 0 =>
          onHeapVector
        case 1 =>
    ...
    benchmark.addCase("On Heap Read Size Only", 10) { _ =>
      readArrays(0)
    }
    benchmark.addCase("Off Heap Read Size Only", 10) { _ =>
      readArrays(1)
    }
    benchmark.addCase("Native Read Size Only", 10) { _ =>
      readArrays(2)
    }

    benchmark.addCase("On Heap Read Elements", 10) { _ =>
      readArrayElements(0)
    }
    benchmark.addCase("Off Heap Read Elements", 10) { _ =>
      readArrayElements(1)
    }
    benchmark.addCase("Native Read Elements", 10) { _ =>
      readArrayElements(2)
    }
  ...
  override def runBenchmarkSuite(mainArgs: Array[String]): Unit = {
    System.load("/Users/xuyiming/native_engine/libvelox.dylib")
