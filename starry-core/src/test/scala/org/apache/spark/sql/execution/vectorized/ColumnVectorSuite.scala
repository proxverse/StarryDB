...
class NativeColumnVectorSuite
  ...
  override protected def test(testName: String, testTags: Tag*)(testFun: => Any)(
    ...
    if (!(testName.startsWith("CachedBatch") || testName.contains("interval"))) {
  ...
  testVectors("string", 10, StringType) { testVector =>
    (0 until 10).map { i =>
      ...
      testVector.putByteArray(i, utf8, 0, utf8.length)
    }
  ...
  testVectors("SPARK-35898: map", 1, mapType) { testVector =>
    ...
    while (i < 6) {
      keys.appendInt(i)
      val utf8 = s"str$i".getBytes("utf8")
      values.putByteArray(i, utf8, 0, utf8.length)
      i += 1
    }
