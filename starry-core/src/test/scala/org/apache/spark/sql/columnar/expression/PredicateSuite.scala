...
class PredicateSuite extends SparkFunSuite with ColumnarExpressionEvalHelper {
  ...
  private def booleanLogicTest(
    ...
    test(s"3VL $name") {
      truthTable.foreach {
        case (l, r, answer) =>
          val expr = op(Literal.create(l, BooleanType),
  ...
  private def checkInAndInSet(in: In, expected: Any): Unit = {
    // expecting all in.list are Literal or Literal.
  ...
  test("basic IN/INSET predicate test") {
    checkInAndInSet(In(Literal.create(null, IntegerType), Seq(Literal(1),
      Literal(2))), null)
    checkInAndInSet(In(Literal.create(null, IntegerType),
      Seq(Literal.create(null, IntegerType))), null)
    checkInAndInSet(In(Literal.create(null, IntegerType), Seq.empty), null)
    checkInAndInSet(In(Literal(1), Seq.empty), false)
    checkInAndInSet(In(Literal(1), Seq(Literal.create(null, IntegerType))), null)
    checkInAndInSet(In(Literal(1), Seq(Literal(1), Literal.create(null, IntegerType))),
      true)
    checkInAndInSet(In(Literal(2), Seq(Literal(1), Literal.create(null, IntegerType))),
      null)
  ...
  test("IN with different types") {
    def testWithRandomDataGeneration(dataType: DataType, nullable: Boolean): Unit = {
      ...
      val input = inputData.map(Literal.create(_, dataType))
    ...
    val atomicTypes: Set[DataType] = DataTypeTestUtils.integralType ++ Set(
      DoubleType,
      FloatType,
      BinaryType,
      BooleanType,
...
//  ignore("SPARK-22705: In should use less global variables") {
...
//  }
  ...
  test("IN/INSET: binary") {
    ...
    val nl = Literal.create(null, onetwo.dataType)
    ...
    val nS = Seq(Literal(Array(1.toByte, 2.toByte)), Literal(Array(3.toByte)),
      Literal.create(null, onetwo.dataType))
  ...
  private val smallValues =
    Seq(1.toByte, 1.toShort, 1, 1L, Decimal(1), Array(1.toByte), Date.valueOf("2000-01-01"),
      new Timestamp(1), "a", 1f, 1d,
//      0f, 0d,
      false,
//      Array(1L, 2L)
    )
      .map(Literal(_))
//  ++ Seq(Literal.create(MyStruct(1L, "b")),
...
//  )
  private val largeValues =
    Seq(2.toByte, 2.toShort, 2, 2L, Decimal(2), Array(2.toByte), Date.valueOf("2000-01-02"),
      new Timestamp(2), "b", 2f, 2d,
//      Float.NaN, Double.NaN,
      true,
//      Array(2L, 1L)
    )
      .map(Literal(_))
//  ++ Seq(Literal.create(MyStruct(2L, "b")),
...
//    checkEvaluation(EqualNullSafe(nullNullType, nullNullType), true)
  ...
  ignore("EqualTo on complex type") {
...
//  }
  ...
  test("SPARK-29100: InSet with empty input set") {
    ...
    val inSet = InSet('a.string, Set.empty)
    checkEvaluation(inSet, false, row, StructType(Seq(StructField("a", IntegerType))))
  }

  ignore("SPARK-32764: compare special double/float values") {
  ...
  ignore("SPARK-32110: compare special double/float values in array") {
  ...
  ignore("SPARK-32110: compare special double/float values in struct") {
