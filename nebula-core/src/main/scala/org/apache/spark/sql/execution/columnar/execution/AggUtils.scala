...
package org.apache.spark.sql.execution.columnar.execution
...
object AggUtils {
  ...
  private def mayRemoveAggFilters(exprs: Seq[AggregateExpression]): Seq[AggregateExpression] = {
    exprs.map { ae =>
      if (ae.filter.isDefined) {
        ae.mode match {
          // Aggregate topicFilters are applicable only in partial/complete modes;
          // this method topicFilters out them, otherwise.
  ...
  private def createAggregate(
    ...
    var forceSortAggregate = forceApplySortAggregate(child.conf)

    if (SortOrder.orderingSatisfies(
          child.outputOrdering,
          groupingExpressions.map(SortOrder(_, Ascending)))) {
      forceSortAggregate = true
    }
  ...
  def planAggregateWithoutDistinct(
    ...
    val interExec: SparkPlan =
      mayAppendMergingSessionExec(groupingExpressions, aggregateExpressions, partialAggregate)
  ...
  def planStreamingAggregation(
    ...
    val restored =
      StateStoreRestoreExec(groupingAttributes, None, stateFormatVersion, partialMerged1)
  ...
  def planStreamingAggregationForSession(
    ...
    if (groupWithoutSessionExpression.isEmpty) {
      throw new AnalysisException(
        "Global aggregation with session window in streaming query" +
          " is not supported.")
    }
    ...
    val restored = SessionWindowStateStoreRestoreExec(
