...
import org.apache.spark.sql.execution.{NebulaConf, WholeStageCodegenExec}
    ...
    (file: PartitionedFile) =>
      {
        ...
        if (enableVectorizedReader && canSupport(requiredSchema)) {
          val vectorizedReader = if (NebulaConf.nativeParquetReaderEnabled) {
          ...
          var iter = if (NebulaConf.asyncNativeParquetReaderEnabled) {
          ...
          try {
            ...
            if (NebulaConf.nativeParquetReaderEnabled) {
              vectorizedReader
