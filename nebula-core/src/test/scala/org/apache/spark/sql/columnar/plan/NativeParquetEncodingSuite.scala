...
import org.apache.spark.sql.execution.datasources.parquet.{NativeVectorizedParquetRecordReader2, ParquetCompatibilityTest}
...
class NativeParquetEncodingSuite extends ParquetCompatibilityTest with ColumnarSharedSparkSession {
  ...
  test("All Types Dictionary") {
    (1000 :: Nil).foreach { n => {
      withTempPath { dir =>
        ...
        val reader = new NativeVectorizedParquetRecordReader2(
          conf.offHeapColumnVectorEnabled, 250)
        reader.initialize(file, null)
        var batch = reader.resultBatch()
...
//        assert(batch.numRows() == n)
        ...
        while (i < n) {
          ...
          assert(batch.column(0).getByte(i%250) == 1)
          assert(batch.column(1).getInt(i%250) == 2)
          assert(batch.column(2).getLong(i%250) == 3)
          assert(batch.column(3).getUTF8String(i%250).toString == "abc")
          assert(batch.column(4).getInt(i%250) == 13)
          assert(batch.column(5).getLong(i%250) == 100000)
          assert(batch.column(6).getBoolean(i%250) == true)
          i += 1
        }
        reader.close()
      }
    ...
    ( 10000 :: Nil).foreach { n => {
      withTempPath { dir =>
        ...
        Range(0, 100).foreach { i =>
          ...
          val conf = sqlContext.conf
          val size = conf.parquetVectorizedReaderBatchSize
          val reader = new NativeVectorizedParquetRecordReader2(
            conf.offHeapColumnVectorEnabled, size)
          reader.initialize(file, null)
...
//          assert(batch.numRows() == n)
          var i = 0
          while (i < n) {
            assert(batch.column(0).isNullAt(i % size))
            assert(batch.column(1).isNullAt(i % size))
            assert(batch.column(2).isNullAt(i % size))
            assert(batch.column(3).isNullAt(i % size))
            assert(batch.column(4).isNullAt(i % size))
            assert(batch.column(5).isNullAt(i % size))
            assert(batch.column(6).isNullAt(i % size))
            i += 1
          }
          reader.close()
        }
